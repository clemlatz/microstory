 ...
 * presentation field — identical setup to `CharacterDescriptionEditor`/
 * `NoteContentEditor`, split into its own file for the same reason
 * (BlockNote/ProseMirror reads `window` on creation and therefore cannot
 * render during Next.js's server-side render; `StoryPresentationEditPage`
 * loads this component via `next/dynamic` with `ssr: false`).
 *
